...
use crate::{
    config::{GUEST_ENTRIES, PSCI_CONTEXT, CPU_NUM},
    ...
    match regs.x[0] as usize {
        PSCI_CPU_ON | PSCI_CPU_HVC_ON => {
            ...
            if let Some(gpm) = &guest_gpms[CPU_TO_VM[cpu_id as usize]] {
                ...
                unsafe {
                    PSCI_CONTEXT[pcpu_id].store(regs.x[3] as usize, Ordering::SeqCst);
                    GUEST_ENTRIES[pcpu_id].store(regs.x[2] as usize, Ordering::SeqCst);
                }
...
fn handle_dabt(vcpu: &mut Vcpu) -> RvmResult {
    ...
    trace!("handling dabt, fault addr 0x{:x}", fault_vaddr);
...
pub fn irq_handler() -> RvmResult {
    ...
    if let Some(irq_id) = pending_irq() {
        // TODO: Judge whether vcpu is running on the cpu.
        // if irq_id != 30 {
